...
#### 3.3. Year
```{r}
paste("The earliest year of host_since is ",min(listings2$host_since),", and the lastest year of host_since is ",max(listings2$host_since),sep='')
paste("The earliest year of first_review is ",min(listings2$first_review),", and the lastest year of first_review is ",max(listings2$first_review),sep='')
paste("The earliest year of last_review is ",min(listings2$last_review),", and the lastest year of last_review is ",max(listings2$last_review),sep='')
```
...
* host_response_rate
```{r}
ggplot(listings4 %>% count(host_response_rate), aes(x = host_response_rate, y = n)) + 
  geom_line()
```
...
Keep variables with the absolute value of correlation bigger than 0
```{r}
...
Drop variables with low t-statistics
  
#### 4.1. Transform ```amenities``` & ```host_verifications``` by counting
```{r,results = 'hide',message=FALSE}
var_nchar <- dput(corr_nchar$var)
listings_nchar <- listings_nchar[,var_nchar]
# Linear Regression Model
fit_nchar <- lm(cx_scores~., listings_nchar)
stargazer(fit_nchar,type='html',title = 'Linear Regression Model', out = 'fit_nchar.html')
```
```{r}
summary(fit_nchar)
```
...
listings_reg_nchar <- listings_nchar %>% 
  dplyr::select(-host_response_rate,-within_an_hour,-accommodates,-accommodates,-beds,-within_a_few_hours,-within_a_few_hours,-Hotel,-host_since_2018,-host_since_2019,-Condominium,-host_since_2010,-Hotel_room,-bedrooms,-North,-host_since_2015,-host_has_profile_pic,-Boutique_hotel,-host_since_2017,-Wong_Tai_Sin,-host_since_2011,-other_property_types,-House,-Serviced_apartment,-within_a_day,-Tuen_Mun,-host_since_2012,-cleaning_fee,-Tai_Po,-Shared_room,-Private_room,-extra_people)
...
#### 4.2. Transform ```amenities``` & ```host_verifications``` using nlp
```{r,results = 'hide',message=FALSE}
var_nlp <- dput(corr_nlp$var)
listings_nlp <- listings_nlp[,var_nlp]

# Linear Regression Model
fit_nlp <- lm(cx_scores~., listings_nlp)
stargazer(fit_nlp,type='html',title = 'Linear Regression Model', out = 'fit_nlp.html')
```
```{r}
summary(fit_nlp)
```
...
listings_reg_nlp <- listings_nlp %>% 
  dplyr::select(-host_response_rate,-within_an_hour,-hair_dryer,-smoke_alarm,-accommodates,-tv,-selfie,-identity_manual,-beds,-laptop_friendly_workspace,-iron,-within_a_few_hours,-jumio,-Hotel,-host_since_2018,-host_since_2019,-government_id,-Condominium,-extra_people,-host_since_2010,-Hotel_room,-bedrooms,-North,-phone,-host_since_2015,-host_has_profile_pic,-heating,-Boutique_hotel,-host_since_2017,-email,-Wong_Tai_Sin,-host_since_2011,-other_property_types,-House,-host_identity_verified,-Serviced_apartment,-within_a_day,-air_conditioning,-offline_government_id,-host_since_2012,-cleaning_fee,-Kwun_Tong,-Tuen_Mun)
...
Test error is the smallest when mtry = 7. Let ntree = 400
```{r}
rf_model <- randomForest(cx_scores ~ . , data = train, mtry = 7, ntree=400)
rf_model
```
```{r}
plot(rf_model)
